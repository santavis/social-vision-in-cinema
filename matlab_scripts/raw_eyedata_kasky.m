...
% GigaTrack: Kasky (Experiment 2): Extract fixation, saccades and blinks as well as pupil size, and fixation coordinates from eye-tracker fixation
...
%
% Severi Santavirta, last modification 8th of August 2023
% ------------------------------------------------------------------------------------------------

%% INPUT

dataset = 'kasky'; % Experiment 2
fixation_report_path = 'path/eyedata/raw/kasky/fixrep_kasky.txt'; % Where is the fixation report?
saccade_report_path = 'path/eyedata/raw/kasky/sacrep_kasky.txt'; % Where is the saccade report?
stimulus_path = 'path/stimulus/kasky/kasky_eyetracking_mp4'; % Where are the stimulus video files?
func_path = 'path/funcs'; % Where are the needed functions?
output = 'path/eyedata/subdata/kasky'; % Where to save all the data?
addpath('path/funcs');
...
if(~exist(sprintf('%s/fixrep_%s_preprocessed.txt',path,dataset),'file'))
    ...
    fixrep.SUBJECT = subs;
    fixrep.BLOCK = blocks;
    fixrep = sortrows(fixrep,252,'ascend'); % Sort the table to subject order
    ...
    for I = 1:size(subs,1)
        ...
        for J = 1:size(blocks,1)
            ...
            if((J<9 && size(subblocktrials,1)<3) || (J==9 && size(subblocktrials,1)<2)) % Kasky had 9 blocks where each block should contain 3 trials, but the last block should contain only 2 blocks.
                nn=nn+1;
...
if(~exist(sprintf('%s/sacrep_%s_preprocessed.txt',path,dataset),'file'))
    ...
    sacrep.SUBJECT = subs;
    sacrep.BLOCK = blocks;
    sacrep = sortrows(sacrep,224,'ascend'); % Sort the table to subject order
...
for I=1:size(clips,1)
    vid_dur_eyetracking(I) = unique(tmp(strcmp(fixrep.SUBJECT,'K01') & fixrep.TRIAL_INDEX==I)); % Hard-coded
end
vid_dur = horzcat(vid_dur_clip,vid_dur_eyetracking);

% CHECKED. For some reason, after the last clip (kasky_26, block 9, trial 2) the subjects have been watching blank screen for about 20 sec which is still recorded.
% For other clips the duration read from the mp4 clips is 0,04 - 1,3sec
% longer than reported in the eye-tracker reports. This is because the avi (original presentation file)
% to mp4 conversion (for lowlevel feature extraction) is not accurate and after conversion the clips can
% continue a short time as a frozen screen (checked by comparing visually the mp4 files and the stimulus files in eyelink software). 
% These timing missmatches are corrected so that the eye-tracking data for kasky_26 is corrected to the
% mp4 file end time and all other clips follow the eye-tracker report
% timings. Then the low-level data extracted from the mp4 files are cut
% from the end to match the eye-tracking data length.
trial_durations = vid_dur_eyetracking;
trial_durations(end) = floor(vid_dur_clip(end)*1000);
...
for I = 1:size(subjects,1)
    if(~exist(sprintf('%s/subjects/%s.mat',output,subjects{I}),'file'))
       ...
       if(check_fix<26 || check_sac <26) % Hard-coded
          ...
          warnings{1,1} = 'Trial amount presented for subject is different than it should be! Data was not saved';
           ...
           % Fixations inside video area (VideoArea kasky: x = [0,1024], y = [96,676])
           [subdata.fix_in_video_area,warning_videoarea] = getInsideVideoArea(subdata.fix_x,subdata.fix_y,[0,1024],[96,676]);  
