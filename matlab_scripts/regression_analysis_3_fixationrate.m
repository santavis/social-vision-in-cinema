%% Regression Analysis: Cross-validated OLS regression (FixationRate X [low-level & mid-level & high-level features)
% The fixation rate is predicted with the clustered low-level, mid-level and high level features.
% Precess:
%           1.  Fixation rate is independently modelled with each cluster
...
analysisName = 'fixationRate_ols_tw1000_shift0'; % Choose the name based on the time window and selected pupil size shift
dependent = 'fixationRate';
...
shift = 0; % How many time windows the Y should be shifted compared to X? (NOT IMPLEMENTED FOR FIXATIONRATE)
...
for d = 1:size(dset,1)
    ...
    for s = 1:size(subs,1)
        ...
        if(s == 1)
            fixationRate_dset = processFixations(fix_end_times,trial,tw);
        else
            fixationRate_dset(:,s) = processFixations(fix_end_times,trial,tw);
        end
