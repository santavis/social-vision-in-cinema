%% Regression Analysis: Cross-validated OLS regression (Blink rate X [low-level & mid-level & high-level features)
% The blink rate is predicted with the clustered low-level, mid-level and high level features.
% Precess:
%           1.  Blink rate is independently modelled with each cluster
...
%                    
%                   i)  To create the null distribution of test set R
...
analysisName = 'blinkRate_ols_tw4000_shift0'; % Choose the name based on the time window and selected pupil size shift
dependent = 'blinkRate';
dset = {'localizer';'kasky';'conjuring'}; % Experiment 1-3
...
tw = 4000; % Sampling time window in milliseconds, shoud be one of these: 100, 200, 500, 1000, 2000, 4000
shift = 0; % How many time windows the Y should be shifted compared to X? (NOT IMPLEMENTED FOR BLINKRATE)

% Permutation parameters
nperm = 500; % How many permutations to run

%% Load and process fixation data and predictors
...
for d = 1:size(dset,1)
    ...
    % Loop over subjects
    for s = 1:size(subs,1)
        fprintf('%s: Reading fixation rate: %d/%d\n',dset{d},s,size(subs,1));
        ...
        if(s == 1)
            blinkRate_dset = processBlinks(blink_end_times,trial,tw);
        else
            blinkRate_dset(:,s) = processBlinks(blink_end_times,trial,tw);
        end
...
% Select only regressors with consistent CV results (same sign for the beta
...
% Function calculates the blink rates for each time window.
%
