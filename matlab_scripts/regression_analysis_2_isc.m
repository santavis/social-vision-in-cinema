%% Regression Analysis: Cross-validated OLS regression (ISC X [low-level & mid-level & high-level features)
% The ISC is predicted with the clustered low-level, mid-level and high level features.
% Precess:
%           1.  ISC is independently modelled with each cluster
...
%                       so the previously validated feature model stays
%                       intact.
...
analysisName = 'isc_ols_tw1000_shift0'; % Choose the name based on the time window and selected pupil size shift
dependent = 'ISC';
...
input_isc = '/Volumes/Research/Neurotiede/Severi/gigatrack/isc/isc_changing';
...
tw = 1000; % Sampling time window in milliseconds, shoud be one of these: 200, 500, 1000, 2000, 4000
shift = 0; % How many time windows the Y should be shifted compared to X? (NOT IMPLEMENTED FOR ISC)

% Permutation parameters
nperm = 500; % How many permutations to run

%% Load and process pupil size data and predictors
% ISC is calculated in time windows and we have one time series over all
% subjects. It is possible that to shift the ISC time series if we assume
% that the affect of the regressors come with a lag.
...
for d = 1:size(dset,1)
    ...
    isc_dset = readtable(sprintf('%s/isc_%d_millisecond_tw_%s.csv',input_isc,tw,dset{d}));
    ...
    nanidx = (sum(isnan(isc'))'>0);
    isc = isc(~nanidx,:);

    % Normalize by dividing with subjectwise means to prevent the possible baselinen
    % difference to have a significant effect on the prediction errors in
    % the testing phase
    isc = isc./mean(isc);
    ...
    % Shift ISC
    %[isc_shift,trial_shift] = shiftISC(isc,trial,shift);
    trial_shift=trial;

    % Collect datasets
    %y{d} = isc_shift;
    trial_dset{d} = trial_shift;
    
    % Define 1ms trials for predictor processing
    f = find_files(sprintf('%s/%s/',input_eyedata,dset{d}),'*.mat');
    eyedata = load(f{1});
    if(d==1) % Localizer has different trials
        trial_1ms = zeros(size(eyedata.subdata.trial_indices,1),1);
        trial_1ms(1:find(eyedata.subdata.trial_indices==17,1,'last')) = 1;
        trial_1ms((find(eyedata.subdata.trial_indices==17,1,'last')+1):find(eyedata.subdata.trial_indices==34,1,'last')) = 2;
        trial_1ms((find(eyedata.subdata.trial_indices==34,1,'last')+1):find(eyedata.subdata.trial_indices==51,1,'last')) = 3;
        trial_1ms((find(eyedata.subdata.trial_indices==51,1,'last')+1):end) = 4;
    else
        trial_1ms = eyedata.subdata.trial_indices;
    end
    trials_1ms_included = ismember(trial_1ms,include_trials{d});
    trial_1ms = trial_1ms(trials_1ms_included);
    
    % Load subjectwise gaze class regressors and transform to dummy
    % variables. "Unknown", "Outside video area" and "animals" are
    % excluded. Not enough animals in the stimulus and others are
    % uninteresting.
    for i = 1:size(subs,1)
        fprintf('%s: Reading eyedata: %d/%d\n',dset{d},i,size(subs,1));
        eyedata = load(sprintf('%s/%s/subjects/%s.mat',input_eyedata,dset{d},subs{i}));
        if(i==1)
            gaze = zeros(size(trial_1ms,1),size(subs,1),6);
        end
        ...
        gaze_dummy = gaze_dummy(trials_1ms_included,:);
    ...
    predictors_lowlevel = predictors_lowlevel(trials_1ms_included,:);
    predictors_lowlevel_processed = processPredictors(predictors_lowlevel,trial_1ms,1,shift,tw);

    % Process gaze class predictors
    predictors_gaze_processed = processGaze(gaze,trial_1ms,shift,tw);
    ...
    predictors_highlevel = predictors_highlevel(trials_1ms_included,:);
    predictors_highlevel_processed = processPredictors(predictors_highlevel,trial_1ms,1,shift,tw);
    ...
    predictor_cuts = predictor_cuts(trials_1ms_included,:);
    predictor_cuts = processCuts(predictor_cuts.cut,trial_1ms,shift,tw);
...
% If the addition of the given regressor did not increase the R significatly,
% the regressor is dropped and ít is concluded that there is not enough evidence that the feature has an effect. 
...
for r = 1:size(predictor_names_m,2)
    ...
    if(pval<0.05)
        ...
        % rounds when adding new regressors into the model (may happen if the regressors are highly vorrelated)
