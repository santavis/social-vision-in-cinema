%% Gaze Prediction Anlaysis: Create heatmaps for the gaze prediction analysis
%
% Process:
%       1. Extract gaze coordinates and change coordinate space from display space to stimulus frame space
%       2. Calculate heatmaps in original stimulus frame space
%       3. Downsample to target resolution for resource management and save trialwise.
%
% Information for heatmap kernel calculations
% Localizer: sigma=1, viewDist = 90, viewWidth = 31, viewWidthPixel = 720, viewHeightPixel = 576, VideoArea in eyetracking coordinates: x = [153,873], y = [97,672]
% Kasky: sigma=1, viewDist = 70 (assumption), viewWidth = 61, viewWidthPixel = 1024, viewHeightPixel = 580, VideoArea in eyetracking coordinates: x = [0,1024], y = [96,676]
% Conjuring: sigma=1, viewDist = 70 (assumption), viewWidth = ,60 viewWidthPixel = 1000, viewHeightPixel = 564, VideoArea in eyetracking coordinates: x = [13,1012], y = [105,668]
%
% Severi Santavirta 17.11.2023
...
targetResolution = [64,64]; % The main analysis resolution
...
display_frame = [768,1024]; % Stimulus frame size
...
% Where are the eye-tracking data?
input = sprintf('path/eyedata/subdata/%s/subjects',dataset); 
...
for I = 1:size(subjects,1)
    ...
    % Define if the coordinate is within display
    x_sub(x_sub>display_frame(2) | x_sub<1) = nan;
    y_sub(y_sub>display_frame(1) | y_sub<1) = nan;
    
    % The stimulus frame is in the middle of the screen, change coordinates
    % to match the stimulus frame
    xCoordShift = (display_frame(2)-stimulus_frame(2))/2+1; 
    yCoordShift = (display_frame(1)-stimulus_frame(1))/2+1; 
    x_sub = x_sub-xCoordShift;
    y_sub = y_sub-yCoordShift;
    
    % Nan if the gaze is outside the stimulus frame
    x_sub(x_sub<1 | x_sub>stimulus_frame(2)) = nan;
    y_sub(y_sub<1 | y_sub>stimulus_frame(2)) = nan;
...
kern = eISC_gaussKernel(sigma,[],viewDist,viewWidth,stimulus_frame(2),1);
...
for w = 1:size(tw,2)
    ...
    heatmaps = cell(size(unique(trial),1),1);
    for tr = 1:size(unique(trial),1)
        formatStrHeatmap= sprintf('%s/%s/tw%d/heatmaps_%s_tw%d_trial_%%0%dd.mat',path_heatmaps,dataset,tw(w),dataset,tw(w),3);
        if(~exist(sprintf(formatStrHeatmap,tr),'file'))
            ...
            trial_heatmaps = zeros([targetResolution,size(t,1)],'single');
            r_trial = zeros(size(t,1),1,'single');
            for ww = 1:size(t,1) % time windows
                fprintf('Calculating ISC: Trial: %d, TW: %d/%d\n',tr,ww,size(t,1));

                % TW indices
                if(ww==size(t,1))
                    idx = ((t(ww)+1):size(x_trial,1))';
                else
                    idx = ((t(ww)+1):t(ww+1))';
                end
                ...
                fixation_heatmap = zeros(stimulus_frame(1),stimulus_frame(2),size(subjects,1));
                for s = 1:size(x_trial,2) % subjects

                    points = zeros(size(idx,1),2);
                    points(:,1) = y_trial(idx,s);
                    points(:,2) = x_trial(idx,s);
                    fixation_heatmap(:,:,s) = eISC_fixationHeatmap(points,kern,stimulus_frame(2),stimulus_frame(1),duration);
                end

                % Calculate isc from the heatmap for weighting observations
                r_trial(ww,1) = single(eISC_spatialSimilarity2(fixation_heatmap));

                % Calculate the population average heatmap
                fixation_heatmap = mean(fixation_heatmap,3,'omitnan');

                % Downsample to analysis resolution
                trial_heatmaps(:,:,ww) = single(imresize(fixation_heatmap, targetResolution, 'bicubic'));

            end

            % Save trial heatmaps
            save(sprintf(formatStrHeatmap,tr),'trial_heatmaps','r_trial');
        end
    end
end
...
display_frame = [768,1024]; % Stimulus display size
...
for I = 1:size(subjects,1)
    ...
    % The stimulus frame is in the middle of the screen, change coordinates
    % to match the stimulus frame. For kasky, the x size of the stimulus do
    % not fit fully to the display so no need to change the coordinates in
    % x axis
    yCoordShift = (display_frame(1)-stimulus_frame(1))/2+1; 
    y_sub = y_sub-yCoordShift;
...
% Create Gaussian kernel (sigma 1 degrees, Nummenmaa 2014 or Lahnakoski 2014))
kern = eISC_gaussKernel(sigma,[],viewDist,viewWidth,display_frame(2),1);
...
for w = 1:size(tw,2)
    ...
    for tr = 1:size(unique(trial),1)
        ...
        if(exist(sprintf(formatStrHeatmap,tr),'file'))
            ...
            trial_heatmaps = zeros([targetResolution,size(t,1)],'single');
            r_trial = zeros(size(t,1),1,'single');
            parfor ww = 1:size(t,1) % time windows
                fprintf('Calculating ISC: Trial: %d, TW: %d/%d\n',tr,ww,size(t,1));
                ...
                fixation_heatmap = zeros(stimulus_frame(1),display_frame(2),size(subjects,1));
                for s = 1:size(x_trial,2) % subjects

                    points = zeros(size(idx,1),2);
                    points(:,1) = y_trial(idx,s);
                    points(:,2) = x_trial(idx,s);
                    fixation_heatmap(:,:,s) = eISC_fixationHeatmap(points,kern,display_frame(2),stimulus_frame(1),duration);
                end
